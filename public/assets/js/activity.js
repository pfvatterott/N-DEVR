  ...
  if (mapTest) {
    ...
    L.tileLayer(
      "https://api.mapbox.com/styles/v1/{id}/tiles/{z}/{x}/{y}?access_token={accessToken}",
      {
        ...
        // hid this!!
        accessToken:
          "pk.eyJ1IjoicGZ2YXR0ZXJvdHQiLCJhIjoiY2tsODZxOW5uMXo3ZTJ4bW1iN3YwbWpsaCJ9.LGIyO-vQru6dyenUYpZE3A",
      }
    ).addTo(mymap);
    ...
    const getSegments = (
      ...
      fetch(
        `/profile/activity/${southWestLat},${southWestLng},${northEastLat},${northEastLng},`,
        {
          method: "GET",
          headers: {
            ...
            if (!idList.includes(data.segments[i].id)) {
              ...
              L.polyline(coordinates, {
                ...
                .on("click", (e) => {
                  ...
                  const oldDeleteButton = document.getElementById(
                    "deleteButton"
                  );
                  if (oldDeleteButton) {
                  ...
                  totalDistance +=
                    Math.round(
                      e.target.options.metaDataDistance * 0.00062137 * 100
                    ) / 100;
                  totalElevation += Math.round(
                    e.target.options.metaDataElevation * 3.28084
                  );
                  totalDistanceEl.textContent =
                    "Total Distance: " + totalDistance + " miles";
                  totalElevationEl.textContent =
                    "Total Elevation: " + totalElevation + " feet";
                  ...
                  p2.textContent =
                    "Elevation: " +
                    Math.round(e.target.options.metaDataElevation * 3.28084) +
                    " feet";
                  li.appendChild(i);
                  li.appendChild(span);
                  li.appendChild(p1);
              ...
              var marker = L.marker(
                [
                  data.segments[i].start_latlng[0],
                  data.segments[i].start_latlng[1],
                ],
                {
                ...
                .bindPopup(
                  "<b>" +
                    data.segments[i].name +
                    "</b> <br> Distance: " +
                    Math.round(0.00062137 * data.segments[i].distance * 100) /
                      100 +
                    " miles <br> Elevation: " +
                    Math.round(data.segments[i].elev_difference * 3.28084) +
                    " ft"
                )
                .addTo(mymap);
            }
    ...
    const createGraph = (data) => {
      if (newGraphData.length > 0) {
        ...
        for (let i = 0; i < data[1].data.length; i++) {
          let xValue =
            Math.round(0.00062137 * data[1].data[i] * 100) / 100 +
            lastSegmentEndDistance;
          newGraphData.push({
            x: xValue,
            y: Math.round(3.28084 * data[2].data[i]),
          });
        }
      } else {
        for (let i = 0; i < data[1].data.length; i++) {
          newGraphData.push({
            x: Math.round(0.00062137 * data[1].data[i] * 100) / 100,
            y: Math.round(3.28084 * data[2].data[i]),
          });
        }
      }
